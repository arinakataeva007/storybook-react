...
const FormatedTextField = ({ placeholder = "", rlValue = "", onChanges, onInput, disabled = false, theme = "light-theme", textRich, tailPlacement })=>{
    const [value, setValue] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(rlValue); // Обновляем начальное значение
    const [maxLength, setMaxLength] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(placeholder.length);
    const [richIsOpen, setOpen] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
    const [onInputs, setOnInputs] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
    const [constPlaceholder, setPlaceholder] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(placeholder);
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(()=>{
        if (value !== "" && rlValue && !onInputs) {
            setPlaceholder(""); // Обновляем placeholder, если значения есть
        } else if (!onInputs) {
            setPlaceholder(placeholder); // Восстанавливаем placeholder, если нет значений
        }
        onChanges(rlValue); // Передаём новое значение в onChanges
    }, [
        rlValue,
        value,
        placeholder,
        onInputs,
        onChanges
    ]); // Добавляем все зависимости, чтобы не было "старая" информация
    const replaceSymbol = (event)=>{
        setOnInputs(true);
        const index = event.currentTarget.value.length - 1;
        if (fieldRef.current) {
            if (event.currentTarget.value === "" || !event.currentTarget.value) {
                setPlaceholder(placeholder); // Восстановить placeholder, если поле пустое
            } else if (constPlaceholder === placeholder || event.currentTarget.value !== "" && constPlaceholder !== "") {
                const arrPlaceholder = constPlaceholder.split("");
                arrPlaceholder[index] = fieldRef.current.value[index];
                setPlaceholder(arrPlaceholder.join("")); // Обновить placeholder, если символ изменился
            }
        }
    };
    const onChangeInput = (event)=>{
        if (rlValue !== "" && rlValue) {
            setPlaceholder(""); // Если есть значение, то placeholder пустой
        } else {
            setPlaceholder(placeholder); // Восстанавливаем placeholder
        }
        onChanges(event.target.value); // Передаем изменённое значение
        setOnInputs(false);
    };
    const closeTooltip = ()=>{
        setOpen((prevState)=>!prevState); // Переключаем видимость подсказки
    };
...
//# sourceMappingURL=stories-input-formated-field-stories.58286ef7394be5d3fb99.hot-update.js.map